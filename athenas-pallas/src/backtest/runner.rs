...
use super::sources::{FxCsvSource, YahooCsvSource};
...
use crate::risk::{PauseCheck, RejectShort, RiskPipeline};
...
impl BacktestRunner {
    ...
    pub fn run_buy_and_hold(cfg: &BacktestConfig) -> crate::Result<BacktestReport> {
        ...
        Self::run_with_strategy(cfg, &mut strategy)
    }
    ...
    pub fn run_with_strategy<S: Strategy>(
        ...
        let risk_checks: Vec<Box<dyn crate::risk::RiskCheck + Send + Sync>> =
            vec![Box::new(PauseCheck), Box::new(RejectShort)];
        let risk = RiskPipeline::new(risk_checks);
        ...
        let mut src = load_source(cfg, exchange.clone(), symbol)?;

        while let Some(ev) = src.next_event() {
            let ts = event_ts(&ev);
            dispatch_event_sync(&mut state, strategy, &risk, &exec, ev, &mut intents)?;
            if let Some(eq) = state.mark_to_market_equity(&cfg.instrument) {
        ...
        let summary = summarize(equity.clone(), cfg.periods_per_year);
        let mut report =
            report_from_summary(summary, state.fill_count, started.elapsed().as_millis() as u64);
...
fn report_from_summary(s: PerformanceSummary, fill_count: u64, wall_time_ms: u64) -> BacktestReport {
    BacktestReport {
        pnl: s.pnl.to_string(),
