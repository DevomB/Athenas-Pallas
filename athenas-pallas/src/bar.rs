...
/// CSV bar replay (`ts,open,high,low,close,volume`).
...
impl CsvBarSource {
    pub fn from_path(path: &Path, exchange: ExchangeId, symbol: Symbol) -> std::io::Result<Self> {
        ...
        for (index, record) in reader.deserialize().enumerate() {
            let row: OhlcvRow = record
                .map_err(|error| std::io::Error::new(std::io::ErrorKind::InvalidData, error))?;
            parse_ts_required(&row.ts, &format!("row {}", index + 2))?;
            rows.push(row);
        }
        if rows.is_empty() {
        ...
        Ok(Self::from_ohlcv_rows(exchange, symbol, rows))
    }

    pub fn from_csv(data: &str, exchange: ExchangeId, symbol: Symbol) -> Result<Self, String> {
        let mut reader = csv::Reader::from_reader(data.as_bytes());
        let mut rows = Vec::new();
        for (index, record) in reader.deserialize().enumerate() {
            let row: OhlcvRow = record.map_err(|error| error.to_string())?;
            parse_timestamp(&row.ts)
                .ok_or_else(|| format!("invalid timestamp row {}: {}", index + 2, row.ts))?;
            rows.push(row);
        }
        if rows.is_empty() {
...
impl HistoricalSource for CsvBarSource {
    fn next_event(&mut self) -> Option<Event> {
        ...
        self.idx += 1;
        let ts = parse_timestamp(&row.ts).expect("timestamp validated while loading CSV");
...
pub fn parse_timestamp(value: &str) -> Option<OffsetDateTime> {
    ...
    if let Ok(timestamp) = PrimitiveDateTime::parse(value, &datetime) {
...
pub(crate) fn parse_ts(value: &str) -> Option<OffsetDateTime> {
    parse_timestamp(value)
}

pub(crate) fn parse_ts_required_err(value: &str, context: &str) -> Result<OffsetDateTime, String> {
    parse_timestamp(value).ok_or_else(|| format!("invalid timestamp {context}: {value}"))
}

fn parse_ts_required(value: &str, context: &str) -> std::io::Result<OffsetDateTime> {
    parse_timestamp(value).ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("invalid timestamp {context}: {value}"),
        )
    })
}
...
impl BarSeries {
    ...
    pub fn infer_periods_per_year(&self, class: crate::instrument::AssetClass) -> f64 {
        use crate::interval::{default_periods_per_year, infer_periods_per_year_from_spacing};
        if self.bars.len() < 2 {
...
impl HistoricalSource for BarSeriesSource {
    fn next_event(&mut self) -> Option<Event> {
